...
Si vous utilisez un capteur de vilebrequin VR, la plaque nécessite l'ajout d'un conditioneur VR. The board has been designed to work with the dual VR conditioner from JBPerf (http://www.jbperf.com/dual_VR/index.html) which will plug directly in. Other VR conditioners will also likely work, but have not been tested. There is now also an official VR board that can be used, see link on the left.
...
Selon le type de capteur de vilebrequin et d'arbre à cames que vous possedez, il y a un nombre de ponts (jumpers) que vous devrez ajuster. Les ponts qui doivent être ajustés sont:
